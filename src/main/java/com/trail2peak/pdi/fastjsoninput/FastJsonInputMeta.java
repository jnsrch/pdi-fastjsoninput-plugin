...
/**
 * Store run-time data on the FastJsonInput step.
 */
public class FastJsonInputMeta extends BaseStepMeta implements
		StepMetaInterface {
	...
	private FastJsonInputField[] inputFields;
	...
	/** Flag : remove source field from result */
	...
	/**
	 * @return the RemoveSourceField flag
	 */
	...
	/**
	 * @param removeSourceField
	 *            the RemoveSourceField flag to set
	 */
	 ...
	 */
	public FastJsonInputField[] getInputFields() {
		return inputFields;
	}
	 ...
	 */
	public void setInputFields(FastJsonInputField[] inputFields) {
	...
	public Object clone() {
		FastJsonInputMeta retval = (FastJsonInputMeta) super.clone();
		...
		for (int i = 0; i < nrFields; i++) {
			if (inputFields[i] != null) {
				retval.inputFields[i] = (FastJsonInputField) inputFields[i]
						.clone();
			}
		}
		return retval;
	...
	public String getXML() {
		...
		retval.append("    ").append(
				XMLHandler.addTagValue("removeSourceField", removeSourceField));
		...
		for (int i = 0; i < fileName.length; i++) {
			...
			retval.append("      ").append(
					XMLHandler.addTagValue("exclude_filemask",
							excludeFileMask[i]));
			retval.append("      ").append(
		...
		retval.append("    ").append(
				XMLHandler.addTagValue("IsInFields", inFields));
		retval.append("    ")
		...
		retval.append("    ").append(
				XMLHandler
						.addTagValue("shortFileFieldName", shortFileFieldName));
		retval.append("    ").append(
				XMLHandler.addTagValue("pathFieldName", pathFieldName));
		retval.append("    ").append(
				XMLHandler.addTagValue("hiddenFieldName", hiddenFieldName));
		retval.append("    ").append(
		...
		retval.append("    ").append(
				XMLHandler.addTagValue("rootUriNameFieldName",
						rootUriNameFieldName));
		retval.append("    ").append(
				XMLHandler
						.addTagValue("extensionFieldName", extensionFieldName));
		retval.append("    ").append(
				XMLHandler.addTagValue("sizeFieldName", sizeFieldName));
	...
	private void readData(Node stepnode) throws KettleXMLException {
		try {
			...
			readurl = "Y".equalsIgnoreCase(XMLHandler.getTagValue(stepnode,
					"readurl"));
			removeSourceField = "Y".equalsIgnoreCase(XMLHandler.getTagValue(
					stepnode, "removeSourceField"));
			...
			for (int i = 0; i < nrFiles; i++) {
				...
				Node filemasknode = XMLHandler.getSubNodeByNr(filenode,
						"filemask", i);
				Node excludefilemasknode = XMLHandler.getSubNodeByNr(filenode,
						"exclude_filemask", i);
				Node fileRequirednode = XMLHandler.getSubNodeByNr(filenode,
						"file_required", i);
				Node includeSubFoldersnode = XMLHandler.getSubNodeByNr(
						filenode, "include_subfolders", i);
				fileName[i] = XMLHandler.getNodeValue(filenamenode);
				fileMask[i] = XMLHandler.getNodeValue(filemasknode);
				excludeFileMask[i] = XMLHandler
						.getNodeValue(excludefilemasknode);
				fileRequired[i] = XMLHandler.getNodeValue(fileRequirednode);
				includeSubFolders[i] = XMLHandler
						.getNodeValue(includeSubFoldersnode);
			}

			for (int i = 0; i < nrFields; i++) {
				Node fnode = XMLHandler.getSubNodeByNr(fields, "field", i);
				FastJsonInputField field = new FastJsonInputField(fnode);
				inputFields[i] = field;
			}
			...
			inFields = "Y".equalsIgnoreCase(XMLHandler.getTagValue(stepnode,
					"IsInFields"));
			IsAFile = "Y".equalsIgnoreCase(XMLHandler.getTagValue(stepnode,
					"IsAFile"));
			valueField = XMLHandler.getTagValue(stepnode, "valueField");
			shortFileFieldName = XMLHandler.getTagValue(stepnode,
					"shortFileFieldName");
			pathFieldName = XMLHandler.getTagValue(stepnode, "pathFieldName");
			hiddenFieldName = XMLHandler.getTagValue(stepnode,
					"hiddenFieldName");
			lastModificationTimeFieldName = XMLHandler.getTagValue(stepnode,
					"lastModificationTimeFieldName");
			uriNameFieldName = XMLHandler.getTagValue(stepnode,
					"uriNameFieldName");
			rootUriNameFieldName = XMLHandler.getTagValue(stepnode,
					"rootUriNameFieldName");
			extensionFieldName = XMLHandler.getTagValue(stepnode,
					"extensionFieldName");
			sizeFieldName = XMLHandler.getTagValue(stepnode, "sizeFieldName");
		} catch (Exception e) {
			throw new KettleXMLException(
					BaseMessages.getString(PKG,
							"FastJsonInputMeta.Exception.ErrorLoadingXML",
							e.toString()));
		}
	}
	...
	public void allocate(int nrfiles, int nrfields) {
		...
		inputFields = new FastJsonInputField[nrfields];
	}
	...
	public void setDefault() {
		...
		for (int i = 0; i < nrFields; i++) {
			inputFields[i] = new FastJsonInputField("field" + (i + 1));
		}
	...
	public void readRep(Repository rep, IMetaStore metaStore, ObjectId id_step,
		...
		try {
			...
			filenameField = rep
					.getStepAttributeString(id_step, "include_field");
			...
			for (int i = 0; i < nrFiles; i++) {
				...
				fileMask[i] = rep.getStepAttributeString(id_step, i,
						"file_mask");
				excludeFileMask[i] = rep.getStepAttributeString(id_step, i,
						"exclude_file_mask");
				fileRequired[i] = rep.getStepAttributeString(id_step, i,
						"file_required");
				includeSubFolders[i] = rep.getStepAttributeString(id_step, i,
						"include_subfolders");
			}
			...
			for (int i = 0; i < nrFields; i++) {
				...
				field.setPrecision((int) rep.getStepAttributeInteger(id_step,
						i, "field_precision"));
				field.setTrimType(FastJsonInputField.getTrimTypeByCode(rep
			...
			shortFileFieldName = rep.getStepAttributeString(id_step,
					"shortFileFieldName");
			pathFieldName = rep
					.getStepAttributeString(id_step, "pathFieldName");
			hiddenFieldName = rep.getStepAttributeString(id_step,
					"hiddenFieldName");
			lastModificationTimeFieldName = rep.getStepAttributeString(id_step,
					"lastModificationTimeFieldName");
			rootUriNameFieldName = rep.getStepAttributeString(id_step,
					"rootUriNameFieldName");
			extensionFieldName = rep.getStepAttributeString(id_step,
					"extensionFieldName");
			sizeFieldName = rep
					.getStepAttributeString(id_step, "sizeFieldName");
		} catch (Exception e) {
			throw new KettleException(BaseMessages.getString(PKG,
					"FastJsonInputMeta.Exception.ErrorReadingRepository"), e);
		}
	}
	...
	public void saveRep(Repository rep, IMetaStore metaStore,
		...
		try {
			...
			rep.saveStepAttribute(id_transformation, id_step, "removeSourceField",
					removeSourceField);
			...
			for (int i = 0; i < fileName.length; i++) {
				...
				rep.saveStepAttribute(id_transformation, id_step, i,
						"exclude_file_mask", excludeFileMask[i]);
				rep.saveStepAttribute(id_transformation, id_step, i,
						"file_required", fileRequired[i]);
				rep.saveStepAttribute(id_transformation, id_step, i,
						"include_subfolders", includeSubFolders[i]);
			}
			...
			rep.saveStepAttribute(id_transformation, id_step, "pathFieldName",
					pathFieldName);
			rep.saveStepAttribute(id_transformation, id_step,
			...
			throw new KettleException(BaseMessages.getString(PKG,
					"FastJsonInputMeta.Exception.ErrorSavingToRepository", ""
							+ id_step), e);
		}
	}
	...
	public void check(List<CheckResultInterface> remarks, TransMeta transMeta,
		...
		if (!isInFields()) {
			...
			if (input.length <= 0) {
				cr = new CheckResult(
						...
						BaseMessages
								.getString(PKG,
										"FastJsonInputMeta.CheckResult.NoInputExpected"),
						stepMeta);
				remarks.add(cr);
			} else {
				cr = new CheckResult(CheckResult.TYPE_RESULT_OK,
						BaseMessages.getString(PKG,
								"FastJsonInputMeta.CheckResult.NoInput"),
						stepMeta);
				remarks.add(cr);
			}
		}
		...
		if (getInputFields().length <= 0) {
			cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
					BaseMessages.getString(PKG,
							"FastJsonInputMeta.CheckResult.NoInputField"),
					stepMeta);
			remarks.add(cr);
		}
		...
		if (isInFields()) {
			if (Const.isEmpty(getFieldValue())) {
				cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
						BaseMessages.getString(PKG,
								"FastJsonInputMeta.CheckResult.NoField"),
						stepMeta);
				remarks.add(cr);
			} else {
				cr = new CheckResult(CheckResult.TYPE_RESULT_OK,
						BaseMessages.getString(PKG,
								"FastJsonInputMeta.CheckResult.FieldOk"),
						stepMeta);
				remarks.add(cr);
			}
		} else {
			...
			if (fileInputList == null || fileInputList.getFiles().size() == 0) {
				cr = new CheckResult(CheckResult.TYPE_RESULT_ERROR,
						BaseMessages.getString(PKG,
								"FastJsonInputMeta.CheckResult.NoFiles"),
						stepMeta);
				remarks.add(cr);
			} else {
				cr = new CheckResult(CheckResult.TYPE_RESULT_OK,
						BaseMessages.getString(PKG,
								"FastJsonInputMeta.CheckResult.FilesOk", ""
	...
	public StepInterface getStep(StepMeta stepMeta,
			...
			Trans trans) {
		return new FastJsonInput(stepMeta, stepDataInterface, cnr, tr, trans);
	}

	public StepDataInterface getStepData() {
		return new FastJsonInputData();
	}
	...
	/**
	 * Since the exported transformation that runs this will reside in a ZIP
	 * file, we can't reference files relatively. So what this does is turn the
	 * name of files into absolute paths OR it simply includes the resource in
	 * the ZIP file. For now, we'll simply turn it into an absolute path and
	...
	public String exportResources(VariableSpace space,
		...
		try {
			...
			// grabbing the file object...
			// In case the name of the file comes from previous steps, forget
			// about this!
			//
			...
			if (!isInFields()) {
				...
				if (fileList.getFiles().size() > 0) {
					for (FileObject fileObject : fileList.getFiles()) {
						// From :
						// ${Internal.Transformation.Filename.Directory}/../foo/bar.xml
						// To : /home/matt/test/files/foo/bar.xml
						//
						// If the file doesn't exist, forget about this effort
						// too!
						//
						if (fileObject.exists()) {
							// Convert to an absolute path and add it to the
							// list.
							//
					...
					//
					fileName = newFilenames.toArray(new String[newFilenames
							.size()]);
					fileMask = new String[newFilenames.size()]; // all null
																// since
																// converted to
																// absolute
																// path.
					fileRequired = new String[newFilenames.size()]; // all null,
																	// turn to
																	// "Y" :
